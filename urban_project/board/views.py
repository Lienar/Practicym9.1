...
def edit_advertisement(request, pk):
    """Функция POST-запроса для изменния статьи"""
    advertisement = Advertisement.objects.get(pk=pk)
    if request.method == "POST":
        form = AdvertisementForm(request.POST)
        if form.is_valid():
            ...
            advertisement.save()
            return redirect('board:advertisement_list')
        '''Обработака Post-запроса'''
    else:
        initial_dict = {'title': advertisement.title, 'content': advertisement.content, 'author': advertisement.author}
        form = AdvertisementForm(initial=initial_dict)
